...
def bounding_box(image, min_area, max_area):
    ...
    dilated_image = cv2.dilate(blurred_image, None, iterations=1)
    display_image(dilated_image, "Dilated Image")
    
    # Apply Canny edge detection to the dilated image
    med_val = np.median(dilated_image)
    lower = int(max(0, 0.7 * med_val))
    upper = int(min(255, 1.3 * med_val))
    edges = cv2.Canny(dilated_image, threshold1=lower, threshold2=upper)
...
img = load_image('mask_RCNN//testing_dataset//tray (51).jpg')
if img is not None:
